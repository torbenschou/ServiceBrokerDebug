...
<#
  ...
  .SAMPLE
  .\BrokerQueueContractUsages.ps1 -SourceInstance .\MySourceInstance -SourceDB mySourceDB -TargetInstance .\myTargetInstance -TargetDB myTargetDB -SampleTime (seconds)
...
Function GetBrokerQueueData([string]$SourceInstance, [string]$SourceDB)
{
    ...
    {
        write-eventlog -logname Application -Source "Queue Problem" -EventId 50001 -Message "Broker Queue Data - BrokerQueueContractUsages.ps1" -EntryType Warning
...
function WriteBrokerQueueData([string]$RepositoryInstance, [string]$RepositoryDatabase, $dataTable)
{
    ...
    try
    {
        ...
        $bc.DestinationTableName = "SolidQ.service_contract_usages"
    ...
    {
        $ex = $_.exception
        write-eventlog -logname Application -Source "Queue Problem" -EventId 50001 -Message "Bulk insert - Broker Queue Data - BrokerQueueContractUsages.ps1" -EntryType Warning
        write-eventlog -logname Application -Source "Queue Problem" -EventId 50001 -Message $_.Exception.Message -EntryType Error
    }
    Finally
    {
